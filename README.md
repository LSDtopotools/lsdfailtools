...
First, you need to install Docker on your machine using the following link https://docs.docker.com/get-docker/
...
1. Create a folder into which to put all the files, then run:

1. `docker run --rm -it -v C:/LSDTopoTools:/LSDTopoTools dockerhub/dockerhub_info`

1. This should pull the repository along with setting up your Docker container ready for analysis

1. You should now be inside the Docker container at the top directory of your cloned repo ready to begin!

1. To escape your docker session at any point use `ctrl-d`

1. Now you need to install the software. Run the following commands:
...
1. Where XXX is the name of the wheel generated by the python line!

1. Rather than navigating to dist/ to check the name you can just autocomplete this using tab autocomplete e.g. type `pip install dis` then hit tab before enter

1. Everything is now ready to run, note you will need to follow these steps everytime you want to use the software.
...
1. Place the Dockerfile in a directory then navigate there via the command line and run:

1. `docker build --tag lsdtt_failtools_docker .`
...
1. Note: this requires a an account on the NASA EartData website (https://urs.earthdata.nasa.gov), make a login and password, click in Applications>Authorized Apps> Approve More Applications and select NASA GESDISC DATA ARCHIVE. This will be used in the PRECIPITATION section for downloading precipitaiton data.
...
These scripts are used to pre-process data for ingestion into calibration and validation steps. 
Process all the input data: Piezometers, Precipitation, Sentinel and Cosmo-SKYMed interferometry data.
